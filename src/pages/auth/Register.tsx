...
export default function Register() {
  ...
  async function signInWithGoogle() {
    const redirectTo = `${import.meta.env.VITE_APP_URL}/rewards/earn-points`;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (credentials.password.length < 8) {
      setErrMessage("Passwords must be at least 8 characters");
    } else if (credentials.password !== credentials.confirmPassword) {
      setErrMessage("Passwords do not match");
    }
    try {
      setLoading(true);
      const emailRedirectTo = `${import.meta.env.VITE_APP_URL}/login`;
      ...
      setErrMessage(error instanceof Error ? error.message : "Signup failed");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-dvh flex justify-center py-5 px-3 items-center bg-linear-to-br from-[#9013fe] to-[#6D28D9]">
      <div className="flex justify-center w-full max-w-105 ">
        <div className="w-full shadow-[0_4px_6px_rgba(0,0,0,0.1)] py-7.5 px-5 lg:p-10 bg-white rounded-[10px] animate-fadeIn h-fit">
          ...
          {successMessage && (
            <div className=" text-[#166534] bg-[#f0fdf4] border-green-500/20   border p-3 rounded-md mb-5 text-[12px] flex items-center gap-2">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
          ...
          <div className="w-full">
            <form onSubmit={handleSubmit} className="w-full text-[#111827]">
              ...
              <div className="relative group w-full mb-5">
                <input
                  ...
                  className=" peer w-full border text-base py-2.75 px-3.5  border-[#EDE9FE] transition-all ease-linear duration-200 rounded-md outline-none focus:border-[#9013fe]"
                  required
              ...
              <button
                type="submit"
                className="w-full text-base h-13.75  flex justify-center gap-2 items-center p-2.75 text-center bg-[#9013FE] text-white  font-medium border-none transition-colors ease-linear duration-200 rounded-[100px] hover:bg-[#6D28D9]"
              >
                {loading && <ClipLoader size={18} color="#ffffff" />}
                Sign up Account
              </button>
            </form>
            ...
            <button
              ...
              className="border py-2.75 px-3.5 text-sm w-full gap-2 text-[#111827] border-[#EDE9FE] rounded-md hover:bg-[#F5F3FF] transition-colors flex items-center justify-center relative"
            >
              {googleLoading && <ClipLoader size={18} color="#000000" />}
              <img
